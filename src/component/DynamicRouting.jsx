...
const Loader = () => {
  return (
    <div className="w-full h-screen flex flex-col justify-center items-center">
      <div className="w-20 h-20 rounded-full border-4 border-t-transparent border-red-500 animate-spin"></div>
    </div>
  );
};
...
export const UserDetails = () => {
  ...
  return (
    <div className="w-full h-screen flex flex-col justify-center items-center">
      ...
      <p className="truncate">Address: {user?.address.city}</p>
        ...
        {users.map((value) => (
          ...
          key={value.id}
           className="p-5 rounded border border-yellow-400 bg-yellow-100 flex flex-col gap-2 shadow">
            ...
            <Link to={`/user/${value.id}`}className="px-4 py-2 rounded bg-blue-500 text-white text-center">View more</Link>
          </div>
        ))}
      </div>
    </div>
